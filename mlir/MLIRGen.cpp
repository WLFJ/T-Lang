...
private:
  ...
  llvm::StringMap<mlir::tc::FuncOp> functionMap;
  ...
  mlir::Location loc(const Location &loc) {
    return mlir::FileLineColLoc::get(builder.getStringAttr("NOT_SUPPORT"), loc.begin.line,
                                     loc.begin.column);
  }
  ...
  mlir::tc::FuncOp mlirGen(PrototypeAST &proto) {
  ...
  mlir::tc::FuncOp mlirGen(FunctionAST &funcAST) {
    // Create a scope in the symbol table to hold variable declarations.
    SymbolTableScopeT varScope(symbolTable);

    // Create an MLIR function for the given prototype.
    builder.setInsertionPointToEnd(theModule.getBody());
    mlir::tc::FuncOp function = mlirGen(*funcAST.getProto());
    if (!function)
      return nullptr;
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    mlir::tc::FuncOp calledFunc = calledFuncIt->second;
  ...
  mlir::Value mlirGen(ExprAST &expr) {
    switch (expr.getKind()) {
    case tc::ExprAST::Expr_BinOp:
      return mlirGen(cast<BinaryExprAST>(expr));
    case tc::ExprAST::Expr_Var:
      return mlirGen(cast<VariableExprAST>(expr));
    case tc::ExprAST::Expr_Literal:
      return mlirGen(cast<LiteralExprAST>(expr));
    case tc::ExprAST::Expr_StructLiteral:
      return mlirGen(cast<StructLiteralExprAST>(expr));
    case tc::ExprAST::Expr_Call:
